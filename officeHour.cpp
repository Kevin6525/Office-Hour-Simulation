...
student::student(int arrival, int question)
{
	...
	priority = rand() % 10 + 1;
	nameGenerator = rand() % 150 + 1;
	subjectGenerator = rand() % 10;
}

void simulateOfficeHour(int questionTime, double probability, int totalTime, int& averageWait, int& averageQuestion, int& averageOver, multimap<string, string> &studentsTopics)
{
	...
	for(currentTime = 1; currentTime <= totalTime; currentTime++)
	{
		...
		if(studentArrival.getProbability() && averageTime.totalStudents() <= 150)
		{
			...
			string name = "Student" + to_string(students.getName());
			string subject = "Subject" + to_string(students.getSubject());
			studentsTopics.insert(pair<string, string> (name, subject));
		}
...
int main()
{
	...
	cin>>probability;
	multimap <string, string> studentsTopics;
	multimap <string, string> :: iterator itr;
	ofstream outputFile;
	string name;
	int count = 1;
	outputFile.open("HW5Pt3output.txt");
	for(int i = 0; i < 10; i ++)
	{
		simulateOfficeHour(20, probability, 60, averageWait, averageQuestion, averageOffice, studentsTopics);
	...
	outputFile<<"Office Simulation Summary"<<endl;
	outputFile<<"\tName\t\tSubjects"<<endl;
	
	for(itr = studentsTopics.begin(); itr != studentsTopics.end(); itr ++)
	{
		name = itr->first;
		count = studentsTopics.count(name);
		outputFile<<"Number of visits for "<<name<<" :"<<count<<endl;
		outputFile<<'\t'<<itr->first<<'\t'<<itr->second<<endl;
		for(int i = 1; i < count; i ++)
		{
			itr ++;
			outputFile<<'\t'<<'\t'<<'\t'<<itr->second<<endl;
		}
	}
	return 0;
}
